...
def rs_blocks(version, error_correction):
    version_blocks = RS_BLOCK_TABLE[version]
    ec_index = RS_BLOCK_OFFSET.get(error_correction)

    if ec_index is None or len(version_blocks) < ec_index:  # pragma: no cover
        raise Exception(
            "bad rs block @ version: %s / error_correction: %s" %
            (version, error_correction))
    rs_block = version_blocks[ec_index]
